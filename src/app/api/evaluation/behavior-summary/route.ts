...
export async function GET() {
  try {
    const response = await fetch(`${PYTHON_SERVER_URL}/api/evaluation/behavior-summary`, {
      signal: AbortSignal.timeout(10000),
      cache: 'no-store',
    });

    if (!response.ok) {
      return NextResponse.json({ error: 'Failed to fetch behavior summary' }, { status: response.status });
    }
