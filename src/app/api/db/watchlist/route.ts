...
export async function GET() {
  try {
    const response = await fetch(`${PYTHON_SERVER_URL}/api/db/watchlist`, {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      return NextResponse.json({ error: result.detail || 'Failed to add to watchlist' }, { status: response.status });
    }
